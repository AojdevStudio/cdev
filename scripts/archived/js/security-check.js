  ...
  checkSensitiveFiles() {
    ...
    if (pkg.files && pkg.files.length > 0) {
      ...
      for (const file of sensitiveFiles) {
        ...
        if (fs.existsSync(filePath)) {
          this.addError(
            `Sensitive directory/file ${file} found! This contains project-specific data.`,
          );
        }
      }
    }
    ...
    try {
      ...
      for (const file of files) {
        if (file.isFile() && file.name.match(/\.(env|key|secret|token|credential)$/)) {
          this.addWarning(
            `Potentially sensitive file found in published directory: ${dirName}/${file.name}`,
          );
        }
      }
    } catch (error) {
  ...
  checkForHardcodedSecrets() {
    ...
    for (const file of sourceFiles) {
      ...
      for (const pattern of secretPatterns) {
        if (pattern.test(content)) {
          this.addError(
            `Potential hardcoded secret found in ${path.relative(this.packageRoot, file)}`,
          );
          break;
        }
      }
