...
describe('HookSelector', () => {
  ...
  describe('applyPreferences', () => {
    ...
    test('filters by minimum importance', () => {
      const allHooks = [...mockCategorizedHooks.tier1, ...mockCategorizedHooks.tier2, ...mockCategorizedHooks.tier3];
  ...
  describe('detectProjectType', () => {
    ...
    test('detects monorepo by lerna.json', async () => {
      fs.pathExists.mockImplementation(path => {
        return path.includes('package.json') || path.includes('lerna.json');
      });
      fs.readJson.mockResolvedValue({});
  ...
  describe('edge cases', () => {
    ...
    test('handles duplicate hook requests', () => {
      ...
      const commitValidatorCount = selected.filter(h => 
        h.name === 'commit-message-validator.py'
      ).length;
