...
`,
  ...
  async createClaudeMD(targetDir, config) {
    const claudeMdContent = `# Claude Code Instructions

This project uses cdev (Claude Development) for enhanced development workflows.
...
- Engineer: ${config.engineerName}
- Default Editor: ${config.defaultEditor}
${config.setupLinear ? '- Linear Integration: Enabled' : ''}

## Hook Configuration

See \`.claude/settings.json\` to customize hook behavior.

## Getting Started

1. Review \`.claude/settings.json\` for hook configuration
2. ${config.setupLinear && !config.linearApiKey ? 'Add your Linear API key to .env' : 'Configuration is ready'}
3. Run \`claude\` to start using Claude Code with hooks
